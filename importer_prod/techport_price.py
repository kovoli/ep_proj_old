...
url = 'http://export.admitad.com/ru/webmaster/websites/1009966/products/export_adv_products/?feed_id=14638&code=8daef5a69b&user=kovoli&template=40759'

r = requests.get(url)

with open('imports/techport_price.xml', 'wb') as xml_file:
    xml_file.write(r.content)

tree = ET.parse('imports/techport_price.xml')
root = tree.getroot()
...
for prod in root.findall('.//offer'):
    ...
    cat_xml = product_data['typePrefix']
    result_contain = fuzz.token_set_ratio(prod_category, cat_xml)

    try:
        if get_product != None:
            product_data['shop_id'] = 10
            del product_data['typePrefix']
            try:
                if result_contain > 50:
                    price_curent = get_product.prices.get(shop_id=10)
